...
for n in range(28,29):
    ...
    if n == 1:
        ...
        filenamelocation = rootdir+'Data_85_85_port1_backsidebubble_port2_normalfrontside_4_1_15.csv'
        parameters = [2, 2, 0.003, 0.08, 0.0048, 0.0048, 0.1, 1, -1000, 'DH_85_85', 10, 50, 'Data_85_85_port1_backsidebubble_port2_normalfrontside_4_1_15', 85, 10]
...
#    AverageRes = numpy.zeros((len(lvlist_length),numbercolumns))
    AverageResSeed = numpy.zeros((20*parameters[11],numbercolumns))
    AverageRes = numpy.zeros((20*parameters[11],numbercolumns))
    counter1 = 0
...
#        timearray.append(date2)
    ...
    for i in range(0,len(AverageResSeed[:,0])-1):
        if AverageResSeed[i,3] < 0.05 and AverageResSeed[i,3] > 0:
